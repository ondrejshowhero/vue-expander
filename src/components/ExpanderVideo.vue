...
export default {
  ...
  mounted() {
                ...
                promise
                  ...
                  .catch(() => {
                    ...
                    video
                      .play()
                      .then(() => {
                        // Autoplay started with muted sound
                      })
                      .catch(() => {
                        // Autoplay failed again
                      });
                  });
              }
            }
    ...
    window.addEventListener("focus", () => {
      if (this.unmutedByUser) this.muted = false;
    });
  },
  created() {
    window.addEventListener("blur", () => {
  ...
  methods: {
    ...
    pauseOtherVideos() {
      ...
      videos.forEach((video) => {
        if (video !== this.$refs.video) video.pause();
      });
    },
    videoPauses() {
      this.playing = false;
    ...
    playButtonClicked() {
      if (!this.unmutedByUser) this.unmutedByUser = true;
      this.muted = false;
      this.togglePlay();
    },
    togglePlay() {
...
mounted() { let video = this.$refs.video; const observer = new
IntersectionObserver( (entries) => { entries.forEach((entry) => { if
(entry.isIntersecting) { if (this.autoplay) { this.pauseOtherVideos();
video.muted = true; video.autoplay = true; let promise = video.play(); if
(promise !== undefined) { promise .then(() => { // Autoplay started }) .catch(()
=> { this.muted = true; video.muted = true; video .play() .then(() => { //
Autoplay started with muted sound }) .catch(() => { // Autoplay failed again });
}); } } } else { video.pause(); } }); }, { threshold: 0.5 } );
observer.observe(video); window.addEventListener("blur", () => { this.muted =
true; }); window.addEventListener("focus", () => { if (this.unmutedByUser)
this.muted = false; }); },
