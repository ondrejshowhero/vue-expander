  ...
  <div class="wrapper">
    <div class="slider" ref="slider">
      <div class="item" v-for="(item, index) in carouselItems" :key="item">
        <div class="inner">
          <div class="text">
            <p class="lead">Top 9: Sønderjyske naturoplevelser</p>
            <h2 class="title">{{ index + 1 }}. {{ item[0] }}</h2>
            <p class="description">
              {{ item[1] }}
            </p>
            <a
              href="https://www.visitsonderjylland.dk/turist/overnatning/camping?utm_source=web-2022&utm_medium=havas&utm_campaign=camping-22"
              target="_blank"
              class="link"
              :data-track="item[0]"
              :data-res-action="`1780-${item[3]}`"
            >
              Læs mere
            </a>
          </div>
